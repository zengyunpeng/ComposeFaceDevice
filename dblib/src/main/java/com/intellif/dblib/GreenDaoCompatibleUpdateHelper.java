...
public class GreenDaoCompatibleUpdateHelper {
    ...
    @SuppressWarnings("all")
    public void compatibleUpdate(Database db, Class<? extends AbstractDao<?, ?>>... daoClasses) {
    ...
    @SafeVarargs
    private static boolean generateNewTablesIfNotExists_withNoExchangeData(Database db, Class<? extends AbstractDao<?, ?>>... daoClasses) {
        return reflectMethod(db, "createTable", true, daoClasses);
    }

    @SafeVarargs
    private static boolean generateTempTables_withExchangeDataFromOldTable(Database db, Class<? extends AbstractDao<?, ?>>... daoClasses) {
        try {
    ...
    @SafeVarargs
    private static boolean dropAllTables(Database db, boolean ifExists, @NonNull Class<? extends AbstractDao<?, ?>>... daoClasses) {
        return reflectMethod(db, "dropTable", ifExists, daoClasses);
    }

    @SafeVarargs
    private static boolean createAllTables_withNoExchangeData(Database db, boolean ifNotExists, @NonNull Class<? extends AbstractDao<?, ?>>... daoClasses) {
     ...
     */
    @SafeVarargs
    private static void restoreData_fromTempTableToNewTable(Database db, Class<? extends AbstractDao<?, ?>>... daoClasses) {
        try {
            for (int i = 0; i < daoClasses.length; i++) {
                DaoConfig daoConfig = new DaoConfig(db, daoClasses[i]);
                String tableName = daoConfig.tablename;
